...
from calculation.severity import calculate_dsv_safra_df
...
def run():
    ...
    for segment_id_precipitation, planting_start_date_str in test_x.values.tolist():
        value_test_count += 1
        print(
            f"=====> [{value_test_count}/{total_values_for_testing}] FOR OCCURRENCE IN: segment_id_precipitation: {segment_id_precipitation} | planting_start_date: {planting_start_date_str}")
        ...
        while severity < threshold_5d:
            severity = calculate_dsv_safra_df(
    ...
    result_df['day_in_harvest'] = test_y['day_in_harvest']
